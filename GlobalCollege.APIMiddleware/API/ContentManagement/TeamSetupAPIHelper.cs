...
namespace GlobalCollege.APIMiddleware
{
    public static class TeamSetupAPIHelper
    {
        ...
        public static async Task<ModuleSummary> GetTeamSetupList()
        {
         ...
         public static async Task<FrontendPageInformation> GetTeamSetupPageAsync(string AreaName, string ControllerName, string ActionName)
         {
             ...
             HttpResponseMessage response = await client.GetAsync(
                 $"api/teamsetup/GetTeamSetupPageAsync?AreaName={AreaName}&ControllerName={ControllerName}&ActionName={ActionName}");
             if (response.IsSuccessStatusCode)
             {
