...
namespace GlobalCollege.Frontend.Areas.ContentManagement.Controllers
{
    ...
    public class ResearchSetupController : Controller
    {
        [ResponseCache(Duration = 1200, VaryByQueryKeys = new[] { "*" })]
        public async Task<IActionResult> Index(string Parameters)
        {
            FrontendPageInformation frontendPageInformation = await ResearchSetupAPIHelper.GetResearchSetupPageAsync("ContentManagement", "ResearchSetup", "Index");
        ...
        {
            FrontendPageInformation frontendPageInformation = await ResearchSetupAPIHelper.GetResearchSetupPageAsync("ContentManagement", "ResearchSetup", "Details");
