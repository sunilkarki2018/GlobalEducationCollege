...
namespace GlobalCollege.Frontend.Areas.ContentManagement.Controllers
{
    ...
    public class FAQSetupController : Controller
    {
        [ResponseCache(Duration = 1200, VaryByQueryKeys = new[] { "*" })]
        public async Task<IActionResult> Index(string Parameters)
        {
            FrontendPageInformation frontendPageInformation = await FAQSetupAPIHelper.GetFAQSetupPageAsync("ContentManagement", "FAQSetup", "Index");
        ...
        {
            FrontendPageInformation frontendPageInformation = await FAQSetupAPIHelper.GetFAQSetupPageAsync("ContentManagement", "FAQSetup", "Details");
