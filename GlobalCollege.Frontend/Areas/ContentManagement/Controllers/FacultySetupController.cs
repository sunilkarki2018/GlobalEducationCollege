...
namespace GlobalCollege.Frontend.Areas.ContentManagement.Controllers
{
    ...
    public class FacultySetupController : Controller
    {
        [ResponseCache(Duration = 1200, VaryByQueryKeys = new[] { "*" })]
        public async Task<IActionResult> Index(string Parameters)
        {
            FrontendPageInformation frontendPageInformation = await FacultySetupAPIHelper.GetFacultySetupPageAsync("ContentManagement", "FacultySetup", "Index");
        ...
        {
            FrontendPageInformation frontendPageInformation = await FacultySetupAPIHelper.GetFacultySetupPageAsync("ContentManagement", "FacultySetup", "Details");
