...
namespace GlobalCollege.Frontend.Areas.ContentManagement.Controllers
{
    ...
    public class AdmissionSetupController : Controller
    {
        [ResponseCache(Duration = 1200, VaryByQueryKeys = new[] { "*" })]
        public async Task<IActionResult> Index(string Parameters)
        {
            FrontendPageInformation frontendPageInformation = await AdmissionSetupAPIHelper.GetAdmissionSetupPageAsync("ContentManagement", "AdmissionSetup", "Index");
        ...
        {
            FrontendPageInformation frontendPageInformation = await AdmissionSetupAPIHelper.GetAdmissionSetupPageAsync("ContentManagement", "AdmissionSetup", "Details");
