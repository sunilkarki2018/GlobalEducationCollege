...
namespace GlobalCollege.Frontend.Areas.ContentManagement.Controllers
{
    ...
    public class CareerSetupController : Controller
    {
        [ResponseCache(Duration = 1200, VaryByQueryKeys = new[] { "*" })]
        public async Task<IActionResult> Index(string Parameters)
        {
            FrontendPageInformation frontendPageInformation = await CareerSetupAPIHelper.GetCareerSetupPageAsync("ContentManagement", "CareerSetup", "Index");
        ...
        {
            FrontendPageInformation frontendPageInformation = await CareerSetupAPIHelper.GetCareerSetupPageAsync("ContentManagement", "CareerSetup", "Details");
