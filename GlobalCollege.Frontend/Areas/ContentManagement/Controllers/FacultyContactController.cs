...
namespace GlobalCollege.Frontend.Areas.ContentManagement.Controllers
{
    ...
    public class FacultyContactController : Controller
    {
        [ResponseCache(Duration = 1200, VaryByQueryKeys = new[] { "*" })]
        public async Task<IActionResult> Index(string Parameters)
        {
            FrontendPageInformation frontendPageInformation = await FacultyContactAPIHelper.GetFacultyContactPageAsync("ContentManagement", "FacultyContact", "Index");
        ...
        {
            FrontendPageInformation frontendPageInformation = await FacultyContactAPIHelper.GetFacultyContactPageAsync("ContentManagement", "FacultyContact", "Details");
