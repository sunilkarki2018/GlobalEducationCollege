...
namespace GlobalCollege.Frontend.Areas.ContentManagement.Controllers
{
    ...
    public class TeamAttributeSetupController : Controller
    {
        [ResponseCache(Duration = 1200, VaryByQueryKeys = new[] { "*" })]
        public async Task<IActionResult> Index(string Parameters)
        {
            FrontendPageInformation frontendPageInformation = await TeamAttributeSetupAPIHelper.GetTeamAttributeSetupPageAsync("ContentManagement", "TeamAttributeSetup", "Index");
        ...
        {
            FrontendPageInformation frontendPageInformation = await TeamAttributeSetupAPIHelper.GetTeamAttributeSetupPageAsync("ContentManagement", "TeamAttributeSetup", "Details");
