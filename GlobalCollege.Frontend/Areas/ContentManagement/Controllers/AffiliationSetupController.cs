...
namespace GlobalCollege.Frontend.Areas.ContentManagement.Controllers
{
    ...
    public class AffiliationSetupController : Controller
    {
        [ResponseCache(Duration = 1200, VaryByQueryKeys = new[] { "*" })]
        public async Task<IActionResult> Index(string Parameters)
        {
            FrontendPageInformation frontendPageInformation = await AffiliationSetupAPIHelper.GetAffiliationSetupPageAsync("ContentManagement", "AffiliationSetup", "Index");
        ...
        {
            FrontendPageInformation frontendPageInformation = await AffiliationSetupAPIHelper.GetAffiliationSetupPageAsync("ContentManagement", "AffiliationSetup", "Details");
