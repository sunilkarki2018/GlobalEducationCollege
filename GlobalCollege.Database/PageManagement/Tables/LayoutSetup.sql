CREATE TABLE [PageManagement].[LayoutSetup] (
    [Id]                 UNIQUEIDENTIFIER NOT NULL,
    [InstitutionSetupId] UNIQUEIDENTIFIER NOT NULL,
    [LayoutName]         NVARCHAR (200)   NOT NULL,
    ...
    [ChangeLog]          XML              NULL,
    CONSTRAINT [PK_PageManagement.LayoutSetup] PRIMARY KEY CLUSTERED ([Id] ASC),
    CONSTRAINT [FK_PageManagement.LayoutSetup_Administrator.ApplicationUser_AuthorisedById] FOREIGN KEY ([AuthorisedById]) REFERENCES [Administrator].[ApplicationUser] ([Id]),
    CONSTRAINT [FK_PageManagement.LayoutSetup_Administrator.ApplicationUser_CreatedById] FOREIGN KEY ([CreatedById]) REFERENCES [Administrator].[ApplicationUser] ([Id]),
    CONSTRAINT [FK_PageManagement.LayoutSetup_Administrator.ApplicationUser_ModifiedById] FOREIGN KEY ([ModifiedById]) REFERENCES [Administrator].[ApplicationUser] ([Id]),
    CONSTRAINT [FK_PageManagement.LayoutSetup_ContentManagement.InstitutionSetup_InstitutionSetupId] FOREIGN KEY ([InstitutionSetupId]) REFERENCES [ContentManagement].[InstitutionSetup] ([Id])
);
...
CREATE NONCLUSTERED INDEX [IX_AuthorisedById]
    ON [PageManagement].[LayoutSetup]([AuthorisedById] ASC);


GO
CREATE NONCLUSTERED INDEX [IX_ModifiedById]
    ON [PageManagement].[LayoutSetup]([ModifiedById] ASC);


GO
CREATE NONCLUSTERED INDEX [IX_CreatedById]
    ON [PageManagement].[LayoutSetup]([CreatedById] ASC);


GO
CREATE NONCLUSTERED INDEX [IX_InstitutionSetupId]
    ON [PageManagement].[LayoutSetup]([InstitutionSetupId] ASC);
