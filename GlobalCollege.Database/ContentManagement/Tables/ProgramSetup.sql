CREATE TABLE [ContentManagement].[ProgramSetup] (
    ...
    [AcademicCalendar]     NVARCHAR (200)   NULL,
    [BroucherDownloadlink] NVARCHAR (MAX)   NULL,
    [ThumbnailImageLink]   NVARCHAR (MAX)   NULL,
    [BannerImageLink]      NVARCHAR (MAX)   NULL,
    [PlacementOrder]       INT              NOT NULL,
    [ProgramLogoLink]      NVARCHAR (MAX)   NULL,
    ...
    [ChangeLog]            XML              NULL,
    CONSTRAINT [PK_ContentManagement.ProgramSetup] PRIMARY KEY CLUSTERED ([Id] ASC),
    CONSTRAINT [FK_ContentManagement.ProgramSetup_Administrator.ApplicationUser_AuthorisedById] FOREIGN KEY ([AuthorisedById]) REFERENCES [Administrator].[ApplicationUser] ([Id]),
    CONSTRAINT [FK_ContentManagement.ProgramSetup_Administrator.ApplicationUser_CreatedById] FOREIGN KEY ([CreatedById]) REFERENCES [Administrator].[ApplicationUser] ([Id]),
    CONSTRAINT [FK_ContentManagement.ProgramSetup_Administrator.ApplicationUser_ModifiedById] FOREIGN KEY ([ModifiedById]) REFERENCES [Administrator].[ApplicationUser] ([Id]),
    CONSTRAINT [FK_ContentManagement.ProgramSetup_ContentManagement.InstitutionSetup_InstitutionSetupId] FOREIGN KEY ([InstitutionSetupId]) REFERENCES [ContentManagement].[InstitutionSetup] ([Id])
);
...
CREATE NONCLUSTERED INDEX [IX_AuthorisedById]
    ON [ContentManagement].[ProgramSetup]([AuthorisedById] ASC);


GO
CREATE NONCLUSTERED INDEX [IX_ModifiedById]
    ON [ContentManagement].[ProgramSetup]([ModifiedById] ASC);


GO
CREATE NONCLUSTERED INDEX [IX_CreatedById]
    ON [ContentManagement].[ProgramSetup]([CreatedById] ASC);


GO
CREATE NONCLUSTERED INDEX [IX_InstitutionSetupId]
    ON [ContentManagement].[ProgramSetup]([InstitutionSetupId] ASC);
