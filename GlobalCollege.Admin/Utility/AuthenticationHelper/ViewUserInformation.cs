    ...
    public class ViewUserInformation
    {
        ...
        public static string Fullname()
        {
            try
            {
                ...
                var name = _data.Claims.Where(c => c.Type.Contains("givenname")).FirstOrDefault();
        ...
        public static string GetCurrentUserProfileImage()
        {
            try
            {
                var User = HttpContext.Current.User;
                var _data = (ClaimsIdentity)User.Identity;
                var userprofile = _data.Claims.Where(c => c.Type.Contains("upn")).FirstOrDefault();
                if (userprofile!=null)
                { return userprofile.Value.ToString(); }
                return string.Format("{0}", "/Image/Profile/defaultUser.jpg");
        ...
        public static string GetMobileNumber()
        {
            try
            {
                ...
                var items = string.Join(",", CurrentRequest.Headers.AllKeys.ToArray());


                return items;

                //var Encrypted_MSISDN = CurrentRequest.Headers.GetValues("x-up-calling-line-id").FirstOrDefault();
                //if (!string.IsNullOrEmpty(Encrypted_MSISDN))
                //{
                //    return MobileNumberInformationHelper.GetMobileNumber(Encrypted_MSISDN);
                //}
                //else
                //{
                //    return null;
